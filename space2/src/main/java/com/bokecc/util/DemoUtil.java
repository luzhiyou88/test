...
public class DemoUtil {
	...
	public static void apiVideos_parseXML(Document document) {
		...
		while (videoElements.hasNext()) {
			try {
				...
				if (image == null)
					image = "";
				String size,createTime;
				try {
					size = videoElement.element("filesize").getTextTrim();
				} catch (Exception e) {
					size="0";
				}
				if (size == null)
					size = "0";
				try {
					createTime = videoElement.element("creation-date").getTextTrim();
				} catch (Exception e) {
					createTime = "";
				}
				if (createTime == null)
				...
				String duration = DateUtil.turnSecondsToTimestring(Integer.parseInt(secondStr));
				...
				double fileSize = DemoUtil.getDouble(size);
				fileSize = fileSize / 1048576;
				BigDecimal b = new BigDecimal(fileSize);
				fileSize = b.setScale(3, BigDecimal.ROUND_HALF_UP).doubleValue();
				video.setSize(fileSize);
				SimpleDateFormat fm=new SimpleDateFormat("yyyy-MM-dd kk:mm:ss");
				if(!"".equals(createTime)){
					video.setCreateTime(fm.parse(createTime));
				}
	...
	public static double getDouble(String floatStr){
		double floatNum = 0;
		try {
			floatNum = Double.parseDouble(floatStr);
		} catch (NumberFormatException e) {
		} catch (NullPointerException e){
		}
		return floatNum;
