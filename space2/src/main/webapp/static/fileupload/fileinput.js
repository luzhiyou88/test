...
(function (factory) {
    ...
    previewCache = {
        ...
        init: function (obj) {
            ...
            previewCache.data[id] = {
                ...
                actions: function (showUpload, showDownload,showDelete, showZoom, showDrag, disabled, url, key) {
                	return obj._renderFileActions(showUpload, showDownload, showDelete, showZoom, showDrag, disabled, url, key, true);
                }
            };
        },
        fetch: function (id) {
        ...
        footer: function (id, i, isDisabled, size) {
            ...
            var config = data.config[i], caption = ifSet('caption', config), width = ifSet('width', config, 'auto'),
                ...
                showDwn = ifSet('showDownload', config, true),
                showZoom = ifSet('showZoom', config, data.showZoom),
                showDrag = ifSet('showDrag', config, data.showDrag), disabled = (url === false) && isDisabled,
                actions = data.isDelete ? data.actions(false, showDwn, showDel, showZoom, showDrag, disabled, url, key) : '',
    ...
    defaultFileActionSettings = {
        ...
        removeTitle: '删除文件',
       /* downloadTitle: '下载文件',*/
       /* downloadFileIcon: '<i class="glyphicon glyphicon-file"></i>',
    ...
    /*tActionDownload = '<button type="button" class="kv-file-download {downloadClass}" ' +
    'title="{downloadTitle}" {dataUrl}{dataKey}>{downloadFileIcon}</button>\n';*/
    ...
    FileInput.prototype = {
        ...
        _listen: function () {
            ...
            handler($cont.find('.fileinput-download:not([disabled])'), 'click', $.proxy(self.clear, self));
        ...
        /*_initDownloadButton: function () {
            ...
            self.$preview.find('.kv-file-download').each(function () {
                var $el = $(this);
                handler($el, 'click', function () {
                    self._fileDownload($el);
                });
            });
        },*/
        /*_fileDownload: function ($btn) {
            var self = this, $preview;
            if (!$btn.length) {
                throw '没有文件可下载!';
            }
            self._raise('filedownload', [$btn.data('key')]);
            return false;
        },*/
           ...
           /* self._initDownloadButton();*/
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                handler($el, 'click', function () {
                    ...
                    self._raise('filesuccessremove', [params.id,params.key, index,
                    function(){
                    	var n, cap;
                    	previewCache.unset(self.id, index);
                        n = previewCache.count(self.id);
        ...
        _initSuccessThumbs: function () {
            ...
            self._getThumbs('.file-preview-success').each(function () {
                var $thumb = $(this), $remove = $thumb.find('.kv-file-remove'),$download=$thumb.find('.kv-file-download');
                $remove.removeAttr('disabled');
                $download.removeAttr('disabled');
                handler($remove, 'click', function () {
                    var out = self._raise('filesuccessremove', 
                    		[$thumb.attr('id'),$thumb.attr('rid'), $thumb.data('fileindex'),function(){
                    			cleanMemory($thumb);
                                if (out === false) {
                                    return;
                                }
                ...
                handler($download, 'click', function () {
                    var out = self._raise('filedownload', 
                    		[$thumb.attr('rid')]);
                });
            });
        },
        ...
        _uploadSingle: function (i, files, allFiles) {
            var self = this, total = self.getFileStack().length, formdata = new FormData(), outData,
                previewId = self.previewInitId + "-" + i, $thumb, chkComplete, $btnUpload, $btnDownload, $btnDelete,
            ...
            if (self.showPreview) {
                ...
                $btnDownload = $thumb.find('.kv-file-download');
        ...
        _readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                if (self.maxFileSize > 0 && eval((fileSize/1024).toFixed(2)) > eval((self.maxFileSize/1024).toFixed(2))) {
                    msg = self.msgSizeTooLarge.replace('{name}', caption)
                        .replace('{size}', (fileSize/1024).toFixed(2))
                        .replace('{maxSize}', (self.maxFileSize/1024).toFixed(2));
        ...
        _renderFileFooter: function (caption, size, width, isError) {
            var self = this, config = self.fileActionSettings, rem = config.showRemove, drg = config.showDrag,
                upl = config.showUpload,dwn = config.showDownload, zoom = config.showZoom, out, template = self._getLayoutTemplate('footer'),
                indicator = isError ? config.indicatorError : config.indicatorNew,
                title = isError ? config.indicatorErrorTitle : config.indicatorNewTitle;
            size = self._getSize(size);
            if (self.isUploadable) {
                out = template.replace(/\{actions}/g, self._renderFileActions(upl,dwn, rem, zoom, drg, false, false, false))
                ...
                out = template.replace(/\{actions}/g,
                    self._renderFileActions(false, false,false, zoom, drg, false, false, false))
                    .replace(/\{caption}/g, caption).replace(/\{size}/g, size).replace(/\{width}/g, width)
        ...
        _renderFileActions: function (showUpload, showDownload, showDelete, showZoom, showDrag, disabled, url, key, isInit) {
            if (!showUpload&& !showDownload && !showDelete && !showZoom && !showDrag) {
                return '';
            }
            var self = this,
                vUrl = url === false ? '' : ' data-url="' + url + '"',
                vKey = key === false ? '' : ' data-key="' + key + '"',
                btnDelete = '', btnDownload = '', btnUpload = '', btnZoom = '', btnDrag = '', css,
           ...
           /* if (showDownload) {
                btnDownload = self._getLayoutTemplate('actionDownload')
                    ...
                    .replace(/\{downloadFileIcon}/g, config.downloadFileIcon)
        ...
        lock: function () {
            ...
            if (self.showDownload) {
                self.$container.find('.fileinput-download').removeClass('hide');
            }
            if (self.showCancel) {
        ...
        cancel: function () {
            ...
            self._getThumbs().each(function () {
                ...
                if (self.filestack[ind] !== undefined) {
                    ...
                    $thumb.find('.kv-file-download').removeClass('disabled').removeAttr('disabled');
                }
                self.unlock();
            });
            return self.$element;
        ...
        disable: function () {
            ...
            self.$container.find(".btn-file, .fileinput-remove, .fileinput-download,.fileinput-upload, .file-preview-frame button").attr(
                "disabled",
                true);
            self._initDragDrop();
            return self.$element;
        ...
        enable: function () {
            ...
            self.$container.find(
                ".btn-file, .fileinput-remove, .fileinput-download, .fileinput-upload, .file-preview-frame button").removeAttr("disabled");
            self._initDragDrop();
            return self.$element;
        },
        upload: function () {
    ...
    $.fn.fileinputLocales.zh = {
        ...
        msgSizeTooLarge: '文件 "{name}" (<b>{size} M</b>) 超过了允许大小 <b>{maxSize} M</b>. 请重新上传!',
        ...
        dropZoneClickTitle: '<br>(点击选择 {files})',
